...
function ProtectedRoute({ component: Component, role }: { component: any, role: string }) {
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center bg-slate-50">Loading application...</div>;
  
  if (!user) return <Redirect to="/login" />;
  
  if (user.role !== role) {
    if (user.role === "district_admin") return <Redirect to="/admin/dashboard" />;
  ...
  return (
    <Switch>
      ...
      <Route path="/facility/attendance">
        {() => <ProtectedRoute component={FacilityAttendance} role="facility_staff" />}
      </Route>
      ...
      <Route path="/admin/redistribution">
        {() => <ProtectedRoute component={AdminRedistribution} role="district_admin" />}
      </Route>
