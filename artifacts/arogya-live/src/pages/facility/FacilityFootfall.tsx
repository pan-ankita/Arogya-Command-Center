...
export default function FacilityFootfall() {
  ...
  const todayFootfall =
    footfall?.filter((f) => f.logDate.startsWith(today)) || [];
  ...
  const handleUpdateCount = (dept: string, newCount: number) => {
    ...
    logFootfall.mutate(
      ...
      },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: getGetFootfallQueryKey({ facilityId }),
          });
        },
        onError: () => {
          toast({
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Track patient visits across departments for today.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {DEPARTMENTS.map((dept) => {
            ...
            const displayCount =
              counts[dept] !== undefined ? counts[dept] : serverCount;

            return (
              <Card
                key={dept}
                className="shadow-sm"
                data-testid={`card-footfall-${dept}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        onClick={() =>
                          handleUpdateCount(dept, displayCount - 1)
                        }
        ...
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>
              Total daily patients across all departments
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
